...
import type { UpdateDrawingRegisterUserPayload } from './hooks';

interface EditDrawingRegisterUserModalProps {
  user: ApiDrawingRegisterUser;
  onClose: () => void;
  onSave: (payload: Omit<UpdateDrawingRegisterUserPayload, 'id'>) => void;
...
/** Mirrors features/users/EditUserModal.tsx minus role/department — Drawing Register accounts have neither. */
export function EditDrawingRegisterUserModal({ user, onClose, onSave, isSubmitting }: EditDrawingRegisterUserModalProps) {
    ...
    >
      <div className="max-h-[88vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white">
        <div className="flex items-start justify-between rounded-t-xl bg-brand-800 px-5 py-4">
          <div>
            <h3 className="text-sm font-bold text-white">Edit Drawing Register User</h3>
