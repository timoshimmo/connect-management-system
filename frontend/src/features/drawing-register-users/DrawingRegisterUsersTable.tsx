...
import { ArrowDown, ArrowUp, KeyRound, Search } from 'lucide-react';
...
interface DrawingRegisterUsersTableProps {
  users: ApiDrawingRegisterUser[];
  onView: (user: ApiDrawingRegisterUser) => void;
  onEdit: (user: ApiDrawingRegisterUser) => void;
  onToggleStatus: (user: ApiDrawingRegisterUser) => void;
  onResetPassword: (user: ApiDrawingRegisterUser) => void;
  headerAction?: ReactNode;
}

/**
 * Mirrors features/users/UsersTable.tsx's search/filter/sort/pagination
 * shape, minus role/department (Drawing Register accounts have neither) and
 * plus a Reset Password action.
 */
export function DrawingRegisterUsersTable({
  users,
  onView,
  onEdit,
  onToggleStatus,
  onResetPassword,
  headerAction,
}: DrawingRegisterUsersTableProps) {
  ...
  const filtered = useMemo(() => {
    ...
    const sorted = [...rows].sort((a, b) => {
      const cmp =
        sortKey === 'name' ? a.name.localeCompare(b.name) : new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime();
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-5 py-3.5">
        <div className="flex items-center gap-3">
          <h2 className="text-sm font-semibold text-gray-900">Drawing Register Users ({filtered.length})</h2>
          {headerAction}
        </div>
            ...
            {pageRows.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-5 py-14 text-center text-sm text-gray-500">
                  No Drawing Register users match your filters.
                </td>
              </tr>
            ) : (
              pageRows.map((u) => (
                <tr key={u.id} className="border-b border-gray-50 last:border-b-0 hover:bg-gray-50/60">
                  ...
                  <td className="px-5 py-3 text-gray-500">{u.jobTitle || '—'}</td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <ActionButton onClick={() => onResetPassword(u)}>
