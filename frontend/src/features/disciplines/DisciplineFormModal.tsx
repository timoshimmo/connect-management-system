...
interface DisciplineFormModalProps {
  ...
  onClose: () => void;
  onSave: (payload: { name: string; status: ApiEntityStatus }) => void;
  isSubmitting?: boolean;
}

/** Create or edit a Discipline — used by Discipline Management in the Admin Dashboard. */
export function DisciplineFormModal({ discipline, onClose, onSave, isSubmitting }: DisciplineFormModalProps) {
  const [name, setName] = useState(discipline?.name ?? '');
  const [status, setStatus] = useState<ApiEntityStatus>(discipline?.status ?? 'Active');
    ...
    >
      <div className="w-full max-w-md overflow-y-auto rounded-xl bg-white">
        <div className="flex items-start justify-between rounded-t-xl bg-brand-800 px-5 py-4">
          <h3 className="text-sm font-bold text-white">{discipline ? 'Edit Discipline' : 'New Discipline'}</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-5">
          ...
          <div>
            ...
            <p className="mt-1 text-[11px] text-gray-400">
              Inactive disciplines drop out of the Discipline dropdown on new Drawing Register documents.
            </p>
          </div>
          ...
          <div className="flex justify-end gap-2.5 border-t border-gray-100 pt-4">
            ...
            >
              {isSubmitting ? 'Saving…' : discipline ? 'Save Changes' : 'Create Discipline'}
            </button>
          </div>
        </form>
      </div>
