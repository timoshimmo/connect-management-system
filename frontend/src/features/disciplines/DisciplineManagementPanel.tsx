...
import type { ApiDiscipline, ApiEntityStatus } from '@/lib/apiTypes';
import {
  useDisciplinesAdminQuery,
...
/** Admin Dashboard's Discipline Management section — Drawing Register-only, view/create/edit/activate-deactivate/search/paginate. */
export function DisciplineManagementPanel() {
  ...
  const { data, isLoading } = useDisciplinesAdminQuery({
  ...
  function handleSave(payload: { name: string; status: ApiEntityStatus }) {
    if (editingDiscipline) {
      updateDiscipline.mutate(
        { id: editingDiscipline.id, ...payload },
        { onSuccess: () => { setFormOpen(false); setEditingDiscipline(null); } }
      );
    } else {
      createDiscipline.mutate(payload, { onSuccess: () => setFormOpen(false) });
    }
  }
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-5 py-3.5">
        <div>
          <h2 className="text-sm font-semibold text-gray-900">Disciplines {pagination ? `(${pagination.total})` : ''}</h2>
          <p className="mt-0.5 text-xs text-gray-400">Used only when creating Drawing Register documents.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => { setEditingDiscipline(null); setFormOpen(true); }}
          ...
          >
            <Plus className="h-3.5 w-3.5" /> New Discipline
          </button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {isLoading ? (
              <tr><td colSpan={3} className="px-5 py-14 text-center text-sm text-gray-500">Loading…</td></tr>
            ) : items.length === 0 ? (
              <tr><td colSpan={3} className="px-5 py-14 text-center text-sm text-gray-500">No disciplines match your filters.</td></tr>
            ) : (
              items.map((d) => (
                <tr key={d.id} className="border-b border-gray-50 last:border-b-0 hover:bg-gray-50/60">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex flex-wrap gap-2">
                      <ActionButton onClick={() => { setEditingDiscipline(d); setFormOpen(true); }}>Edit</ActionButton>
                      <ActionButton
                        variant={d.status === 'Active' ? 'danger' : 'primary'}
                        onClick={() =>
                          updateDiscipline.mutate({ id: d.id, status: d.status === 'Active' ? 'Inactive' : 'Active' })
                        }
                      >
      ...
      {formOpen && (
        <DisciplineFormModal
          ...
          onClose={() => { setFormOpen(false); setEditingDiscipline(null); }}
          onSave={handleSave}
          isSubmitting={createDiscipline.isPending || updateDiscipline.isPending}
        />
      )}
    </div>
  );
