 ...
 * assigns both after the draft is submitted. Destination determines which
 * storefront the document appears on once published — Read Site (public)
 * or Drawing Register (its own separately-authenticated accounts).
 */
export function NewDocumentModal({ onClose, onCreate, isSubmitting }: NewDocumentModalProps) {
  ...
  function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!canSubmit || !department || !type || !destination) return;
    onCreate({ title: title.trim(), department, type, description: description.trim(), destination, file });
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-5">
          <div>
            ...
            <input
              ...
              placeholder="e.g. HSE Safety Procedure v1"
          ...
          <div>
            ...
            <p className="mt-1 text-[11px] text-gray-400">
              Where this document appears once approved and published.
            </p>
          </div>
          ...
          <div>
            <label className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-gray-600">
              <Paperclip className="h-3.5 w-3.5" /> Attach Document File (optional)
            </label>
